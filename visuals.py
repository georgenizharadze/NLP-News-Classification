...
def evaluate(results, accuracy, f1):
    ...
    for k, learner in enumerate(sorted(results.keys())):
        for j, metric in enumerate(['train_time', 'acc_train', 'f_train', 'pred_time', 'acc_test', 'f_test']):
            for i in np.arange(3):
                ...
                ax[int(j/3), j%3].set_xticklabels(["25%", "60%", "100%"])
    ...
    patches = []
    for i, learner in enumerate(sorted(results.keys())):
...
def feature_plot(importances, X_train, y_train):
    ...
    # Creat the plot
    fig = pl.figure(figsize = (18,10)) #was (9,5)
    pl.title("Normalized Weights for Top Ten Predictive Features", fontsize = 16)
